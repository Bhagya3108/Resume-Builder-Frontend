  ...
  const handleUpdateResume=async()=>{
    try{
      ...
      setUpdateResume(result?.data)
       swal({title: "SUCCESS ",text: "RESUME UPDATED!",icon: "success",});
      handleClose()
        ...
        <Box sx={style}>
          ...
          <Typography id="modal-modal-description" sx={{ mt: 2 }}>
            ...
            <div className="d-flex justify-content-between">
              {userInput?.skills?.length>0 && userInput?.skills?.map(skill=>(<span className='btn btn-primary d-flex align-items-center justify-content-center'>{skill} <button onClick={()=>removeSkill(skill)} className='btn text-light'><sup>X</sup> </button></span>))}
